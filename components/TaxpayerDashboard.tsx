...
import { Home, FileText, HelpCircle, LogOut, User as UserIcon, MapPin, Calendar, CheckCircle, Phone, AlertCircle } from 'lucide-react';
...
export function TaxpayerDashboard({ user, onLogout }:  TaxpayerDashboardProps) {
  ...
  const [hasPendingPayment] = useState(false);
    ...
    <div className="flex h-screen" style={{ backgroundColor: '#f8f9fa' }}>
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-8">
          ...
          {activeView === 'file-return' && <FileReturnWizard userTaxCategory={taxpayerData.taxCategory} />}
          {activeView === 'payment-history' && <PaymentHistory />}
          {activeView === 'support' && <SupportTickets userType="taxpayer" currentUserTIN={user.id} />}
          
          {/* PROFILE SECTION - WITHOUT TAX PAYMENT HISTORY */}
          {activeView === 'profile' && (
